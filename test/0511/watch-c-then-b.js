...
const LOGFILE = path.join(OUTDIR, 'watch-c-then-b.log');
...
async function runVariant(variant, mid, kw, skw, title) {
  const deviceLog = `/sdcard/logcat_${variant}_0511b.txt`;
  const localLog = path.join(OUTDIR, `logcat_${variant}_0511b.txt`);

  log(`=== 변형 ${variant} 시작 (2차 실행) ===`);
...
async function waitForServiceStop(label, deviceLog) {
  log(`${label} 서비스 종료 대기 중 (이미 실행 중)...`);
  let loops = 0;
  while (serviceRunning()) {
    await sleep(60000);
    loops++;
...
(async () => {
  ...
  // C가 이미 실행 중 — 종료 대기
  await waitForServiceStop('C', '/sdcard/logcat_C_0511.txt');

  // C 로그 수집
  log('C 로그 수집');
  spawnSync('adb', ['-s', DEVICE, 'pull', '/sdcard/logcat_C_0511.txt',
    path.join(OUTDIR, 'logcat_C_0511_r2.txt')], { encoding: 'utf8' });
